...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existing, error: fetchErr } = await admin.from('api_keys').select('user_id').eq('id', id).single();
  if (fetchErr) return NextResponse.json({ error: fetchErr.message }, { status: 500 });
  if (!existing || existing.user_id !== user.id) return NextResponse.json({ error: 'Not allowed' }, { status: 403 });
